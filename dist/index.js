...
program
    ...
    .action(async (options) => {
    try {
        const { result, format } = await executeValidatedInteractiveCommand(options.format, async () => executeCommand('doctor', 'run', { cwd: process.cwd() }));
...
program
    ...
    .argument('<action>', 'Action to perform (list, get, create, update, delete, reply-list, reply-create, reply-update, reply-delete)')
