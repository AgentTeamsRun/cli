...
function isConfiguredProjectInitResult(result) {
    if (!result || typeof result !== 'object')
        return false;
    const r = result;
    return (r.success === true &&
...
function printAgentFiles(agentFiles) {
 ...
 * the initialization.
 */
function printMcpRegistration(mcp) {
    if (!mcp)
        return;
    console.log('');
...
export function printInitResult(result, format) {
    ...
    if (result.authMode === 'personal-token') {
        if (result.personalLogin) {
            ...
            if (!result.personalLogin.persisted) {
                ...
                console.log('✓ Login stored in a file only your account can read (~/.agentteams/credentials); no long-lived key was written to this repository.');
                console.log("  This machine's OS credential store was unavailable. File permissions are weaker than the OS keyring — run 'agentteams auth status' for details.");
            }
            else {
    ...
    if (writtenAgentFiles.length > 0) {
        ...
        if (exampleAgentFiles.length > 0) {
            console.log(`     Merge each -example file into the file it sits next to (${exampleAgentFiles
        ...
        console.log("     Create one with 'agentteams init --agent-files CLAUDE.md' so agents load .agentteams/convention.md.");
    }
