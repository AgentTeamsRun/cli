...
function formatLogin(result) {
    ...
    if (isFileBackend(result.storeBackend))
        lines.push(FILE_BACKEND_WARNING);
    ...
    if (result.deviceAuth === true)
        lines.push('Flow: device code (--device-auth)');
...
function formatStatus(result) {
    ...
    if (isFileBackend(token.storeBackend))
        lines.push(FILE_BACKEND_WARNING);
