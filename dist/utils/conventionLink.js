import { chmodSync, closeSync, constants as fsConstants, existsSync, fsyncSync, lstatSync, mkdirSync, openSync, readFileSync, realpathSync, renameSync, rmSync, statSync, symlinkSync, writeFileSync, } from 'node:fs';
 ...
 */
export const LEGACY_CONVENTION_REFERENCES = [
    `---
alwaysApply: true
---
...
export function toAnchoredExcludePattern(relativePath) {
...
export function resolveGitCommonDir(repoDir) {
...
function unsafeMetadataIssue(code, path, description) {
    return {
        code,
        path,
...
function prepareMetadataPath(commonDir, directoryName, fileName, issueCode) {
...
function writeFileAtomically(path, content, mode) {
    ...
    try {
        ...
        fileDescriptor = openSync(tempPath, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | noFollow, mode);
...
function isTrackedInRepo(repoDir, relativePath) {
    try {
...
function ensureEntryPointParent(repoDir, relativePath) {
    ...
    for (const segment of parentRelativePath.split(/[\\/]/).filter(Boolean)) {
        ...
        try {
            ...
            if (error instanceof Error && error.message.includes('is a symlink or is not a directory'))
                throw error;
 ...
 */
export function upgradeLegacyConventionReference(fullPath) {
    let content;
    let mode;
    try {
    ...
    if (!LEGACY_CONVENTION_REFERENCES.includes(content))
        return false;
    try {
...
export function ensureConventionEntryPoints(repoDir, relativePaths, options) {
    const entries = [];
    const issues = [];
    let changedCount = 0;
    for (const relativePath of relativePaths) {
        ...
        if (pathExists) {
            ...
            if (upgraded)
                changedCount += 1;
            const state = isTrackedInRepo(repoDir, relativePath) ? 'tracked' : 'existing';
            let compatible = true;
        ...
        try {
            ...
            const noFollow = fsConstants.O_NOFOLLOW ?? 0;
            const fileDescriptor = openSync(fullPath, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL | noFollow, 0o644);
            try {
 ...
 */
export function inspectConventionLink(rootDir, repoDir) {
 ...
 */
export function ensureConventionLink(rootDir, repoDir) {
    const linkPath = join(repoDir, CONFIG_DIR);
 ...
 */
export function ensureLocalExclude(repoDir, patterns) {
    ...
    try {
        ...
        writeFileAtomically(excludePath, existing + separator + missing.map((pattern) => `${pattern}\n`).join(''), metadataPath.mode);
 ...
 * git runs hooks from.
 */
function isDefaultHooksPath(repoDir, commonDir, hooksPath) {
 ...
 */
export function ensurePostCheckoutHook(repoDir) {
    const commonDir = resolveGitCommonDir(repoDir);
    if (!commonDir) {
        return {
            status: 'blocked',
...
//# sourceMappingURL=conventionLink.js.map