 ...
 */
export const normalizeProjectRootPath = (absolutePath) => {
    const withForwardSlashes = absolutePath.replace(/\\/g, '/');
    const withoutTrailingSlash = withForwardSlashes.replace(/\/+$/, '');
    return withoutTrailingSlash.length > 0 ? withoutTrailingSlash : '/';
};
export const hashProjectRootPath = (absolutePath) => createHash('sha256').update(normalizeProjectRootPath(absolutePath), 'utf8').digest('hex');
/**
 ...
 */
export const resolveProjectRootHash = (startDir = process.cwd()) => {
...
//# sourceMappingURL=projectRootHash.js.map