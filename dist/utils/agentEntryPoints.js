...
export const AGENT_ENTRY_POINT_VALUES = AGENT_ENTRY_POINT_FILES.map((file) => file.value);
 ...
 */
const AGENT_CLIENT_MARKER_DIRS = {
    'CLAUDE.md': ['.claude'],
 ...
 */
export const AGENT_ENTRY_POINT_FALLBACK = ['CLAUDE.md'];
 ...
 */
export function detectAgentEntryPointFiles(cwd) {
    const detected = AGENT_ENTRY_POINT_VALUES.filter((value) => AGENT_CLIENT_MARKER_DIRS[value].some((marker) => isDirectory(join(cwd, marker))) || isFile(join(cwd, value)));
 ...
 */
export function parseAgentFilesOption(raw) {
    ...
    for (const token of tokens) {
        ...
        if (!match) {
            throw new Error(`Unknown --agent-files value: '${token}'. Valid values: ${AGENT_ENTRY_POINT_VALUES.join(', ')}, ${AGENT_FILES_NONE}.`);
        }
...
//# sourceMappingURL=agentEntryPoints.js.map