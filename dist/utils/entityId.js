 ...
 */
function isEntityIdOptionKey(key) {
...
function isEntityIdListOptionKey(key) {
    return key.endsWith('Ids');
}
function normalizeEntityIdValue(key, value) {
    if (!isEntityIdListOptionKey(key))
        return stripEntityIdPrefix(value);
    return value
        .split(',')
 ...
 */
export function normalizeEntityIdOptions(options) {
    let mutated = false;
    const next = { ...options };
    for (const key of Object.keys(options)) {
        if (!isEntityIdOptionKey(key))
            continue;
