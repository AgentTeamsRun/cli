const hasRemoteDependency = (html) => {
    return (/\b(?:src|href)\s*=\s*["']https?:\/\//i.test(html) ||
        /@import\s+(?:url\()?["']?https?:\/\//i.test(html) ||
        /url\(\s*["']?https?:\/\//i.test(html));
};
export const validatePlanPreviewHtmlSafety = (html) => {
    const reasons = [];
    ...
    if (!/@media\s*\(\s*prefers-color-scheme\s*:\s*dark\s*\)/i.test(trimmedHtml) &&
...
//# sourceMappingURL=planPreviewHtmlSafety.js.map