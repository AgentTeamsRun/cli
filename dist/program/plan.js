...
export function registerPlanCommand(program) {
    ...
    const addLeaf = (name, description, configure = (command) => command) => {
        const command = addOutputOptions(configure(planCommand.command(name).description(description))).addHelpText('after', CONVENTION_HINT);
    ...
    addLeaf('list', 'List plans', (command) => addPaginationOptions(command)
    ...
    addLeaf('get', 'Get a plan', (command) => command.option('--id <id>', 'Plan ID').option('--include-deps', 'Include dependencies in plan output', false));
    addLeaf('status', 'Get plan lifecycle status', (command) => command.option('--id <id>', 'Plan ID'));
    addLeaf('set-status', 'Set plan lifecycle status', (command) => command.option('--id <id>', 'Plan ID').option('--status <status>', 'Plan status'));
    addLeaf('start', 'Start plan execution', (command) => addGitToggleOption(command)
    ...
    addLeaf('finish', 'Finish a plan and optionally attach a completion report', (command) => addCompletionReportMetricsOptions(command)
        .option('--id <id>', 'Plan ID')
        .option('--task <text>', 'Task summary')
        ...
        .option('--model <model>', 'Model ID snapshot')
        .option('--fast', 'Request fast mode for the selected model when supported', false));
    addLeaf('create', 'Create a plan', (command) => addGitToggleOption(command)
        .option('--title <title>', 'Plan title')
        ...
        .option('--model <model>', 'Model ID snapshot')
        .option('--fast', 'Request fast mode for the selected model when supported', false));
    addLeaf('update', 'Update a plan', (command) => command
        .option('--id <id>', 'Plan ID')
        .option('--title <title>', 'Plan title')
        .option('--content <content>', 'Plan content (plain text or Tiptap JSON)')
        .option('--interpret-escapes', 'Interpret \\n sequences in --content as newlines', false)
    ...
    addLeaf('cleanup', 'Remove downloaded plan runbooks', (command) => command.option('--id <id>', 'Plan ID (omit to remove all downloaded plans)'));
    addLeaf('quick', 'Create and finish a quick log', (command) => addCompletionReportMetricsOptions(command)
        .option('--title <title>', 'Plan title')
        .option('--content <content>', 'Plan content')
        .option('--interpret-escapes', 'Interpret \\n sequences in --content as newlines', false)
        .option('--file <path>', 'Read plan content from a local file')
        .option('--keep-temp', 'Keep uploaded files under .agentteams/cli/temp/ instead of deleting them after upload', false)
        .option('--type <type>', 'Plan type (FEATURE, BUG_FIX, ISSUE, REFACTOR, CHORE)')
        .option('--complexity <level>', 'Plan complexity (MINIMAL, STANDARD, FULL)')
        .option('--priority <priority>', 'Plan priority (LOW, MEDIUM, HIGH)')
        .option('--repository-remote-url <url>', 'Repository remote origin URL (defaults to git origin)')
    ...
    addLeaf('link-issue', 'Link an external issue to a plan', (command) => command
        .alias('issue')
        .option('--id <id>', 'Plan ID')
    ...
    addLeaf('unlink-issue', 'Unlink an external issue from a plan', (command) => command.option('--id <id>', 'Plan ID').option('--issue-id <id>', 'PlanExternalIssue ID'));
