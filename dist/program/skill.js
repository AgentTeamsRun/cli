...
export function registerSkillCommand(program) {
    ...
    const addPackageSource = (command) => addCwd(command)
    ...
    addLeaf('list', 'List project skills', (command) => addCwd(command)
    ...
    addLeaf('download', 'Sync every skill package into .agentteams/skills/', (command) => addCwd(command)
        .option('--skill-targets <targets>', "Mirror targets: comma-separated list of agents,claude,github or 'none' (default: detected from marker directories)")
    ...
    addLeaf('create', 'Create a skill from a local package directory', (command) => addPackageSource(command)
    ...
    addLeaf('update', 'Replace a skill package from a local directory', (command) => addPackageSource(command).option('--id <id>', 'Skill ID').option('--scope <scope>', 'PROJECT or PERSONAL'));
    addLeaf('delete', 'Delete a skill', (command) => addCwd(command).option('--id <id>', 'Skill ID').option('--apply', 'Apply the deletion on the server', false));
}
