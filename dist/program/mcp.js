...
export function registerMcpCommand(program) {
    ...
    addMcpRegistrationOptions(mcpCommand
        .command('install')
        .description('Register AgentTeams with the MCP clients detected on this machine. Without --client it applies to every detected client at project scope; --scope user writes machine-wide configs and needs --yes, as does --json at any scope.'))
        .option('--dry-run', "Print the detection plan without writing a file or running a registration command (detection may still run a client's own `--help` to confirm its identity)", false)
