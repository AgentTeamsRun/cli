...
export function registerChangeSetCommand(program) {
    const root = program
        .command('change-set')
    ...
    addLeaf('add-item', 'Add a change set item', (command) => command
    ...
    addLeaf('remove-item', 'Remove a change set item', (command) => command.option('--change-set-id <id>', 'Change set ID').option('--item-id <id>', 'Change set item ID'));
}
