import { CONVENTION_HINT, DEVICE_AUTH_OPTION_DESCRIPTION, DEVICE_AUTH_SET_DEFAULT_DESCRIPTION, executeCommand, formatAuthResultText, handleError, normalizeFormat, printCommandResult, } from './shared.js';
...
export function registerAuthCommand(program) {
    ...
    const addLeaf = (action, description, configure = (command) => command) => {
        const configured = configure(root.command(action).description(description));
        const command = addOutputOptions(configured.option('--format <format>', 'Output format (json; defaults to human-readable text)')).addHelpText('after', CONVENTION_HINT);
    ...
    addLeaf('login', 'Log in to AgentTeams', (command) => command
    ...
    addLeaf('logout', 'Log out of AgentTeams', (command) => command
