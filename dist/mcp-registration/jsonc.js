...
function scanValue(text, index) {
    ...
    if (char === undefined)
        throw new McpConfigParseError('Unexpected end of input');
    if (char === '"')
    ...
    if (i === start)
        throw new McpConfigParseError(`Unexpected token "${char}"`);
    return i;
}
/** List the direct members of the object whose `{` sits at `objectStart`. */
function listMembers(text, objectStart) {
    const members = [];
    ...
    while (i < text.length && text[i] !== '}') {
        ...
        if (text[i] !== '"')
            throw new McpConfigParseError('Expected a quoted object key');
        const keyStart = i;
        const keyEnd = scanString(text, i);
        const key = JSON.parse(text.slice(keyStart, keyEnd));
        const colon = skipTrivia(text, keyEnd);
        if (text[colon] !== ':')
            throw new McpConfigParseError(`Expected ":" after object key "${key}"`);
...
function lineIndentOf(text, index) {
...
function appendMember(text, objectStart, memberText, memberIndent) {
 ...
 */
export function upsertContainerEntry(source, options) {
    ...
    if (!container) {
        ...
        const memberText = `${JSON.stringify(containerKey)}: {\n` +
    ...
    const memberIndent = entries.length > 0
 ...
 * not an object.
 */
export function ensureRootMember(source, key, value) {
    const indentUnit = detectIndentUnit(source);
    if (source.trim().length === 0) {
    ...
    if (members.some((member) => member.key === key)) {
        return { text: source, existed: true, existing: parsed[key] };
    }
    const memberIndent = lineIndentOf(source, rootStart) + indentUnit;
...
export function readContainerEntry(source, containerKey, entryKey) {
    const parsed = parseJsonc(source);
