...
import { DEFAULT_API_URL, findProjectConfig, saveConfig } from '../utils/config.js';
import { getPersonalTokenClient } from '../auth/personalTokenClient.js';
import { BACKUP_SUFFIX, PROJECT_SCOPE_FILE_MODE, USER_SCOPE_FILE_MODE, writeConfigFileAtomically, } from './atomicWrite.js';
...
function countKeyMaterial(source) {
    return source.match(KEY_PATTERN)?.length ?? 0;
}
function redactKeyMaterial(value) {
...
function buildCandidates(context) {
    ...
    for (const client of MCP_CLIENTS) {
        for (const scope of ['user', 'project']) {
            ...
            const candidate = candidates.get(path) ?? { path, kind: 'mcp-config', definitions: [] };
...
async function hasValidPersonalCredential(apiUrl) {
...
async function cleanProjectConfig(candidate, dependencies) {
    ...
    if (config.authMode !== 'personal-token') {
        return {
            path: displayPath,
            outcome: 'SKIPPED',
            detail: "The project has not completed personal-token migration. Run 'agentteams auth login' before removing its only credential.",
        };
    }
    ...
    if (!(await credentialCheck(apiUrl))) {
        return {
            path: displayPath,
            outcome: 'SKIPPED',
            detail: "No valid personal credential is available. Run 'agentteams auth login' and retry; the legacy apiKey was preserved.",
        };
    }
    const next = { ...config };
    delete next.apiKey;
    try {
        saveConfig(candidate.path, next);
        return {
            path: displayPath,
            outcome: 'CLEANED',
...
function cleanMcpConfig(candidate, context) {
    const displayPath = redactKeyMaterial(candidate.path);
    const editable = candidate.definitions.find((definition) => definition.format !== 'toml' && typeof definition.containerKey === 'string');
    ...
    try {
        ...
        return {
            path: displayPath,
            outcome: 'SKIPPED',
            detail: error instanceof McpConfigParseError
    ...
    if (existingEntry === undefined) {
        return {
            path: displayPath,
            outcome: 'SKIPPED',
            detail: 'AgentTeams is not registered in this config, so nothing was inserted; the key material it holds belongs to another tool and needs manual removal.',
        };
    }
...
function cleanBackup(candidate) {
    const displayPath = redactKeyMaterial(candidate.path);
    if (!existsSync(candidate.path)) {
...
export async function runMcpDoctorCommand(options, dependencies = {}) {
    ...
    for (const finding of initial.findings.filter((item) => item.kind === 'project-config')) {
        ...
        if (candidate)
            cleanup.push(await cleanProjectConfig(candidate, dependencies));
    }
    for (const finding of initial.findings.filter((item) => item.kind === 'mcp-config')) {
        const candidate = candidateByPath.get(finding.path);
        if (candidate)
            cleanup.push(cleanMcpConfig(candidate, context));
    }
    for (const finding of initial.findings.filter((item) => item.kind === 'backup')) {
        const candidate = candidateByPath.get(finding.path);
        if (candidate)
            cleanup.push(cleanBackup(candidate));
    }
