...
import { getCredentialStore } from '../auth/credentialStore.js';
...
import { DEFAULT_API_URL, CredentialResolutionError, describeUnusableCredential, findProjectConfig, globalConfigPath, isDeviceAuthDefaultEnabled, loadProjectConfig, resolveCredential, setDeviceAuthDefault, setProjectAuthMode, } from '../utils/config.js';
 ...
 */
export const DEVICE_AUTH_HINT = "If you cannot open that URL here, run 'agentteams auth login --device-auth' to authorize with a short code on another device instead.";
export const DEVICE_AUTH_INIT_HINT = "If you cannot open that URL here, run 'agentteams init --device-auth' to authorize with a short code on another device instead.";
 ...
 */
export function shouldUseDeviceAuth(options, userHomeDir) {
 ...
 */
function assertCredentialStoreUsable() {
    const status = getCredentialStore().status();
    if (status.reason === 'OK')
        return;
    const cause = status.reason === 'NO_BACKEND'
    ...
    const optOutAdvice = isFileCredentialFallbackDisabled(process.env)
        ...
        : 'Or use ';
    throw new PersonalTokenError('STORE_UNAVAILABLE', `Cannot start device authorization: ${cause}${diagnostic}. ` +
 ...
 */
export function decorateLoopbackTimeout(error, hint) {
...
export function describeAuthStatusProblem(resolutionError, state, projectConnected) {
