...
import { parseEntityRef } from '../utils/entityRef.js';
 ...
 */
async function resolveFile(parsed, context) {
...
async function resolveRecord(parsed, context) {
    const { apiUrl, projectId, headers } = context;
    switch (parsed.refType) {
        case 'codeReview':
 ...
 */
function resolveLocalPath(parsed) {
 ...
 */
export async function executeResolveCommand(options, loadContext) {
