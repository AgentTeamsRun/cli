...
import { createReport, deleteReport, dismissReportReview, getReport, listReports, updateReport, } from '../api/report.js';
...
export async function executeReportCommand(apiUrl, headers, action, options) {
    ...
    if (configPath)
        pruneStaleCacheFiles(resolve(configPath, '..', '..'));
    switch (action) {
        case 'list': {
            const params = {};
            if (options.planId)
        ...
        case 'create': {
            ...
            if (!toNonEmptyString(options.planId)) {
                throw new Error('--plan-id is required for report create. A completion report must be linked to a plan. ' +
        ...
        case 'update': {
            ...
            if (reviewRecommendation !== undefined || reviewReason !== undefined) {
                const notice = reviewRecommendation === 'NOT_NEEDED'
        ...
        case 'dismiss-review': {
            if (!options.id)
                throw new Error('--id is required for report dismiss-review');
            return withSpinner('Dismissing report review...', async () => {
                try {
                    ...
                    if (isAxiosError(error)) {
                        ...
                        const errorDetailCode = typeof errorData?.errorDetailCode === 'string' ? errorData.errorDetailCode : undefined;
                        ...
                        if (error.response?.status === 409 && errorDetailCode === 'COMPLETION_REPORT_REVIEW_ALREADY_LINKED') {
                            throw new Error(`An active (non-cancelled) code review is already linked, so the report review cannot be dismissed.${details}`);
                        }
