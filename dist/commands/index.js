...
async function loadRequiredConfig(overrides) {
...
async function executeCommandWithContext(resource, action, options) {
    switch (resource) {
        ...
        case 'skill': {
            ...
            const { apiUrl, headers } = resolveApiContext(config);
            return withApiErrorContext(apiUrl, () => executeSkillCommand(apiUrl, config.projectId, headers, action, {
                ...options,
        ...
        case 'linear': {
            const config = await loadRequiredConfig(buildConfigOverrides(options));
            const { apiUrl, headers } = resolveApiContext(config);
            return withApiErrorContext(apiUrl, () => executeLinearCommand(apiUrl, config.projectId, headers, action, options));
        }
        case 'sentry': {
            const config = await loadRequiredConfig(buildConfigOverrides(options));
            const { apiUrl, headers } = resolveApiContext(config);
            return withApiErrorContext(apiUrl, () => executeSentryCommand(apiUrl, config.projectId, headers, action, options));
        }
        case 'attachment': {
            const config = await loadRequiredConfig(buildConfigOverrides(options));
            const { apiUrl, headers } = resolveApiContext(config);
