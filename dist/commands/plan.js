...
export async function executePlanCommand(apiUrl, projectId, headers, action, options) {
    switch (action) {
        ...
        case 'quick': {
            ...
            // 2. Start plan
            const quickStartBody = { assignedTo: assignAgent };
            if (options.runnerType)
                quickStartBody.runnerType = options.runnerType;
            if (options.model)
                quickStartBody.model = options.model;
            await withSpinner('Starting plan...', () => startPlanLifecycle(apiUrl, projectId, headers, planId, quickStartBody), 'Plan started');
            // 3. Finish plan (no completion report for quick plans)
            const quickFinishBody = {};
            if (options.runnerType)
                quickFinishBody.runnerType = options.runnerType;
            if (options.model)
                quickFinishBody.model = options.model;
            const finishResult = await withSpinner('Finishing plan...', () => finishPlanLifecycle(apiUrl, projectId, headers, planId, quickFinishBody), 'Plan finished');
            return {
