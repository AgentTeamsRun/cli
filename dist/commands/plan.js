...
import { assignPlan, createPlan, deletePlan, finishPlanLifecycle, getPlan, getPlanDependencies, getPlanStatus, linkOriginIssue, listOriginIssues, listPlans, patchPlanStatus, startPlanLifecycle, unlinkOriginIssue, updatePlan, uploadPlanHtml, } from '../api/plan.js';
...
export async function executePlanCommand(apiUrl, projectId, headers, action, options) {
    ...
    switch (action) {
        ...
        case 'start': {
            ...
            const assignAgent = options.agent ?? options.defaultCreatedBy;
            if (!assignAgent) {
        ...
        case 'quick': {
            ...
            // 2. Start plan
            await withSpinner('Starting plan...', () => startPlanLifecycle(apiUrl, projectId, headers, planId, {
                assignedTo: assignAgent,
                runnerType: options.runnerType,
                model: options.model,
                fastMode: options.fast === true,
            ...
            const finishResult = await withSpinner('Finishing plan...', () => finishPlanLifecycle(apiUrl, projectId, headers, planId, finishBody), 'Plan finished');
            if (options.reportFile)
                deleteIfTempFile(options.reportFile, { keep: options.keepTemp });
