...
import { ensureUrlProtocol, interpretEscapes, stripFrontmatter, toNonEmptyString, toPositiveInteger, toSafeFileName, deleteIfTempFile, pruneStaleCacheFiles, } from '../utils/parsers.js';
...
import { createPlan, deletePlan, finishPlanLifecycle, getPlan, getPlanRunbook, getPlanDependencies, getPlanStatus, linkOriginIssue, listOriginIssues, listPlans, patchPlanStatus, quickPlan, startPlanLifecycle, unlinkOriginIssue, updatePlan, uploadPlanHtml, } from '../api/plan.js';
const PLAN_COMPLEXITY_VALUES = PLAN_COMPLEXITY_ORDER;
/**
 ...
 */
export function resolveAgentAssignee(options) {
...
function warnOnComplexityMismatch(complexity, content) {
...
function assertPlanPreviewHtmlSafety(html) {
    const result = validatePlanPreviewHtmlSafety(html);
    if (!result.ok) {
        throw new Error('Plan HTML preview is not theme-safe. ' + `Fix the preview before uploading: ${result.reasons.join('; ')}.`);
    }
}
export function assertComplexityReasonCanBeRecorded(complexityReason, nextComplexity, currentComplexity) {
    if (!complexityReason)
        return;
    if (!nextComplexity) {
...
export const getPlanComplexityValues = () => PLAN_COMPLEXITY_VALUES;
...
export function addTaskIdCommentsToPlanRunbook(markdown, tasks, contentVersion) {
    if (contentVersion !== 'V2' || tasks.length === 0)
        return markdown;
    ...
    for (const line of lines) {
        ...
        const taskHeading = task
            ? new RegExp(`^###\\s+${taskNumber}\\.\\s+${escapeRegExp(task.title)}\\s+—\\s+${escapeRegExp(task.status)}(?:\\s+\\(Wave\\s+\\d+\\))?\\s*$`)
            : null;
...
export function buildPlanTaskSidecar(planId, tasks) {
    const orderedTasks = [...tasks].sort((a, b) => a.orderIndex - b.orderIndex);
...
export function readPlanHtmlUploadInput(options) {
...
export function hasPlanHtmlPreviewInput(options) {
...
export function readPlanHtmlPreviewInput(options) {
    const hasFile = typeof options.htmlFile === 'string' && options.htmlFile.trim().length > 0;
    const hasStdin = options.htmlStdin === true;
    if (hasFile && hasStdin) {
    ...
    const html = hasFile
        ? (() => {
            const filePath = resolve(options.htmlFile);
            if (!existsSync(filePath)) {
...
async function uploadPlanHtmlPreview(apiUrl, projectId, headers, planId, html, sourceLabel, action) {
    try {
        await withSpinner('Uploading plan HTML preview...', () => uploadPlanHtml(apiUrl, projectId, headers, planId, {
            html,
        ...
        throw new Error(`Plan ${planId} was ${action}, but uploading the HTML preview failed (partial failure: the plan body and preview are now out of sync). ` +
...
function extractPlanStatus(result) {
...
function extractCompletionReport(result) {
    if (!result || typeof result !== 'object')
        return undefined;
    const data = result.data;
    if (!data || typeof data !== 'object')
        return undefined;
    const report = data.completionReport;
...
export function buildQuickPlanResult(planId, createResult, finishResult) {
...
export async function executePlanCommand(apiUrl, projectId, headers, action, options) {
    ...
    switch (action) {
        ...
        case 'upload-html': {
            if (!options.id)
                throw new Error('--id is required for plan upload-html');
            const html = readPlanHtmlUploadInput({ file: options.file, stdin: options.stdin });
            if (options.file)
                printFileInfo(options.file, html);
            return withSpinner('Uploading plan HTML summary...', () => uploadPlanHtml(apiUrl, projectId, headers, options.id, {
                html,
                curationType: 'AI_CURATED',
        ...
        case 'finish': {
            ...
            if (includeCompletionReport) {
                ...
                if (payload) {
                    const repositoryRemoteUrl = toNonEmptyString(options.repositoryRemoteUrl) ??
            ...
            if (options.reportFile)
                deleteIfTempFile(options.reportFile, { keep: options.keepTemp });
            return finishResult;
        }
        case 'create': {
            if (!options.title)
            ...
            const repositoryRemoteUrl = toNonEmptyString(options.repositoryRemoteUrl) ?? (options.git === false ? undefined : getGitRemoteOriginUrl());
            ...
            if (options.file)
                deleteIfTempFile(options.file, { keep: options.keepTemp });
            ...
            if (createHtmlContent && createdPlanId) {
                ...
                if (options.htmlFile)
                    deleteIfTempFile(options.htmlFile, { keep: options.keepTemp });
            }
        ...
        case 'update': {
            ...
            const updateResult = await withSpinner('Updating plan...', () => updatePlan(apiUrl, projectId, headers, options.id, body), 'Plan updated');
            if (options.file)
                deleteIfTempFile(options.file, { keep: options.keepTemp });
        ...
        case 'download': {
            ...
            const result = await withSpinner('Downloading plan...', async () => {
                const response = (await getPlanRunbook(apiUrl, projectId, headers, options.id));
                ...
                return {
                    ...
                    sidecarPath: plan.contentVersion === 'V2' && tasks.length > 0
        ...
        case 'quick': {
            ...
            const quickResult = await withSpinner('Completing quick log...', () => quickPlan(apiUrl, projectId, headers, quickBody), 'Quick log completed');
            if (hasQuickFile)
                deleteIfTempFile(options.file, { keep: options.keepTemp });
            if (options.reportFile)
                deleteIfTempFile(options.reportFile, { keep: options.keepTemp });
            const planId = quickResult?.data?.id ?? quickResult?.data?.plan?.id;
            if (!planId) {
