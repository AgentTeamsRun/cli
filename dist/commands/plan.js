...
import { EXECUTION_SNAPSHOT_HINT, resolveExecutionSnapshot, resolveSessionAgentConfigId, } from '../utils/agentIdentity.js';
import { createPlan, deletePlan, finishPlanLifecycle, getPlan, getPlanRunbook, getPlanDependencies, getPlanStatus, linkOriginIssue, listOriginIssues, listPlans, patchPlanStatus, quickPlan, startPlanLifecycle, unlinkOriginIssue, updatePlan, } from '../api/plan.js';
    ...
    switch (action) {
        ...
        case 'finish': {
            ...
            if (includeCompletionReport && (!finishSnapshot.runnerType || !finishSnapshot.model)) {
                throw new Error('--runner-type and --model are required when attaching a completion report.' + EXECUTION_SNAPSHOT_HINT);
            }
            const body = {};
            if (options.task) {
                body.task = options.task;
        ...
        case 'link-issue': {
            ...
            if (!externalUrl && provider !== 'SENTRY')
