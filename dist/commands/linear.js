 ...
 */
export async function executeLinearCommand(apiUrl, projectId, headers, action, options) {
    switch (action) {
        case 'issue-get': {
            if (!options.issueId) {
        ...
        case 'issue-create': {
            ...
            return createLinearIssue(apiUrl, headers, options.title, options.description, options.state, options.teamId, options.parentId, projectId);
        }
        case 'issue-update': {
            if (!options.issueId) {
