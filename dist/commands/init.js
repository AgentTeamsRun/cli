...
import { DEFAULT_CONVENTION_REFERENCE, upgradeLegacyConventionReference, ensurePostCheckoutHook, } from '../utils/conventionLink.js';
 ...
 */
async function runDeviceAuthSetup(context) {
 ...
 */
async function runMcpRegistrationStep(cwd, dependencies) {
    try {
        const { applyMcpBatchInstall } = await import('../mcp-registration/index.js');
        const outcome = applyMcpBatchInstall({ scope: 'project' }, { ...dependencies, context: { cwd, ...(dependencies?.context ?? {}) } });
        return {
            scope: 'project',
...
async function runConfiguredProjectInit(cwd, executionContext, adapterOptions, mcpOptions) {
...
async function executeInitCommandWithContext(options) {
    ...
    if (useDeviceAuth && authMode === 'api-key') {
        throw new Error('Initialization failed: --device-auth cannot be combined with --auth api-key. ' +
    ...
    if (executionContext.kind === 'configured-project') {
        try {
            return await runConfiguredProjectInit(cwd, executionContext, { ...adapterOptions, allowPrompt: false }, { requested: options?.mcp === true, dependencies: options?.mcpDependencies });
        }
        catch (error) {
