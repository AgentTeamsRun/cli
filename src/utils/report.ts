...
export function parseReportOptions(
  ...
  if (qualityScore !== undefined) payload.qualityScore = qualityScore;
  if (commitHash !== undefined) payload.commitHash = commitHash;
  if (branchName !== undefined) payload.branchName = branchName;
  if (filesModified !== undefined) payload.filesModified = filesModified;
  if (linesAdded !== undefined) payload.linesAdded = linesAdded;
  if (linesDeleted !== undefined) payload.linesDeleted = linesDeleted;
  if (durationSeconds !== undefined) payload.durationSeconds = durationSeconds;
  if (commitStart !== undefined) payload.commitStart = commitStart;
  if (commitEnd !== undefined) payload.commitEnd = commitEnd;
  if (pullRequestId !== undefined) payload.pullRequestId = pullRequestId;
  if (reviewRecommendation !== undefined) payload.reviewRecommendation = reviewRecommendation;
  if (reviewReason !== undefined) payload.reviewReason = reviewReason;
