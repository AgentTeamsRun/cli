...
export function registerSentryCommand(program: Command): void {
  const root = program
    .command('sentry')
  ...
  addJsonResourceLeaf(
    ...
    'sentry',
    'list',
    'List Sentry issues from the selected project',
    (command) =>
    ...
    { connection: false, commandAction: 'issue-list' },
  );
  addJsonResourceLeaf(
    issue,
    'sentry',
    'get',
    'Get a Sentry issue',
    (command) => addConnection(command).option('--issue-id <id>', 'Canonical numeric Sentry issue ID'),
