...
export function registerDependencyCommand(program: Command): void {
  ...
  const addLeaf = (action: string, description: string, configure: (command: Command) => Command) =>
    addJsonResourceLeaf(root, 'dependency', action, description, configure, {
      connection: false,
    });
  addLeaf('list', 'List plan dependencies', (command) => command.option('--plan-id <id>', 'Plan ID'));
  addLeaf('create', 'Create a plan dependency', (command) =>
    command.option('--plan-id <id>', 'Plan ID').option('--blocking-plan-id <id>', 'Blocking plan ID'),
  );
  addLeaf('delete', 'Delete a plan dependency', (command) =>
    command.option('--plan-id <id>', 'Plan ID').option('--dep-id <id>', 'Dependency ID to delete'),
  );
}
