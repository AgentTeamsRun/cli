...
export function registerCommentCommand(program: Command): void {
  ...
  const addMutationOptions = (command: Command) =>
    addWriteOptions(command).option('--expected-updated-at <iso>', "The comment's updatedAt as you last read it");

  addLeaf('list', 'List comments', (command) =>
    addPaginationOptions(addTargetOptions(command)).option(
      '--type <type>',
      'Comment type (RISK, MODIFICATION, GENERAL)',
    ),
  );
  addLeaf('get', 'Get a comment', (command) => command.option('--id <id>', 'Comment ID'));
  addLeaf('create', 'Create a comment', (command) =>
    addWriteOptions(addTargetOptions(command))
  ...
  addLeaf('update', 'Update a comment', (command) =>
  ...
  addLeaf('reply-list', 'List replies', (command) =>
    addPaginationOptions(command.option('--id <id>', 'Parent comment ID')),
  );
  addLeaf('reply-get', 'Get a reply', (command) => command.option('--reply-id <id>', 'Reply ID'));
  addLeaf('reply-create', 'Create a reply', (command) =>
    addWriteOptions(command).option('--id <id>', 'Parent comment ID').option('--content <content>', 'Reply content'),
  );
  addLeaf('reply-update', 'Update a reply', (command) =>
    addMutationOptions(command).option('--reply-id <id>', 'Reply ID').option('--content <content>', 'Reply content'),
  );
  addLeaf('reply-delete', 'Delete a reply', (command) =>
    addMutationOptions(command).option('--reply-id <id>', 'Reply ID'),
  );
}
