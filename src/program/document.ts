...
export function registerDocumentCommand(program: Command): void {
  ...
  const addWriteContractOptions = (command: Command, includeExpectedUpdatedAt = false): Command => {
    const configured = command
      .option(
        '--guide-hash <hash>',
        'Hash of the document guide you followed. Stale hash is rejected with GUIDE_OUTDATED.',
      )
      .option(
        '--idempotency-key <key>',
        'Retry-safe key. Repeating the same key with the same request replays the first result.',
      );
    return includeExpectedUpdatedAt
      ? configured.option(
          '--expected-updated-at <iso>',
          "The document's updatedAt as you last read it. Rejects the write if it changed meanwhile.",
        )
      : configured;
  };

  addLeaf('create', 'Create a document', (command) =>
    addWriteContractOptions(command)
  ...
  addLeaf('update', 'Update a document', (command) =>
    addWriteContractOptions(command, true)
  ...
  addLeaf('list', 'List documents', (command) =>
  ...
  addLeaf('tags', 'List confirmed document tags', (command) =>
    command
      .option('--visibility <visibility>', 'Document visibility: PRIVATE or PROJECT')
      .option('--archived <state>', 'Archive filter (ACTIVE, ARCHIVED, ALL)'),
  );
  addLeaf('delete', 'Delete a document', (command) =>
    addWriteContractOptions(command, true).option('--id <id>', 'Document ID'),
  );
  ...
  addLeaf('revision-get', 'Get a document revision', (command) =>
    command.option('--id <id>', 'Document ID').option('--revision-id <id>', 'Document revision ID'),
  );
  addLeaf('revision-restore', 'Restore a document revision', (command) =>
    command.option('--id <id>', 'Document ID').option('--revision-id <id>', 'Document revision ID'),
  );
  addLeaf('comment-list', 'List document comments', (command) =>
    addPaginationOptions(command)
      .option('--id <id>', 'Document ID')
      .option('--order <order>', 'Comment order (asc or desc)'),
  );
  addLeaf('comment-create', 'Create a document comment', (command) =>
    addWriteContractOptions(command)
      .option('--id <id>', 'Document ID')
      .option('--content <text>', 'Markdown comment content')
      .option('--file <path>', 'Read markdown comment content from a local file'),
  );
  addLeaf('comment-update', 'Update a document comment', (command) =>
    addWriteContractOptions(command, true)
      .option('--id <id>', 'Document ID')
  ...
  addLeaf('comment-delete', 'Delete a document comment', (command) =>
    addWriteContractOptions(command, true)
      .option('--id <id>', 'Document ID')
      .option('--comment-id <id>', 'Document comment ID'),
  );
