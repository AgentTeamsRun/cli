...
function suggestFallback(client: McpClientDefinition, requested: ToolProfile): ToolProfile {
  ...
  if (declared && rejectedToolNames(client, declared).length === 0) return declared;
 ...
 */
export function resolveClientToolProfile(
  client: McpClientDefinition,
  requested: ToolProfile,
  explicit: boolean,
): ResolvedClientToolProfile {
  const rejected = rejectedToolNames(client, requested);
  if (rejected.length === 0) return { toolProfile: requested };
...
export function isExplicitToolProfile(value: unknown): boolean {
