...
export type FileCredentialSaveResult = { ok: true } | { ok: false; detail: string };
...
export interface FileCredentialStore {
   ...
   */
  check(options?: { create?: boolean }): ProtectionCheck;
...
export function isFileCredentialFallbackDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
...
export type ProtectionCheck = { ok: true } | { ok: false; detail: string };
 ...
 * without root.
 */
export function checkPosixProtection(
  facts: PosixNodeFacts,
  expected: { kind: 'file' | 'directory'; uid: number },
  options: {
    /**
...
export function parseAclEntries(output: string): AclEntry[] {
 ...
 */
function grantsOnly(subject: string, principal: string, path: string | null): boolean {
  ...
  if (path !== null) {
    ...
    if (found.startsWith(prefix)) return found.slice(prefix.length) === expected;
 ...
 */
export function verifyAclOutput(output: string, principal: string, path: string): ProtectionCheck {
...
export function buildAclHardenCommand(path: string, principal: string, kind: 'file' | 'directory'): CredentialCommand {
...
export function buildAclInspectCommand(path: string): CredentialCommand {
  return { command: 'icacls', args: [path] };
}

/** `DOMAIN\user`, the form `icacls` prints. */
function resolveWindowsPrincipal(env: NodeJS.ProcessEnv): string | null {
...
function fileSystemDetail(error: unknown): string {
...
export function createFileCredentialStore(options: CreateFileCredentialStoreOptions = {}): FileCredentialStore {
  ...
  const pathFor = (account: string): string => join(directory, credentialFileName(account));

  const runIcacls = (command: CredentialCommand): { ok: boolean; stdout: string } => {
    if (!runner) return { ok: false, stdout: '' };
    const result = runner(command);
    return { ok: result.status === 0, stdout: result.stdout };
  };

  /** Set, then prove, the protection on a path that already exists. */
  const protectPath = (path: string, kind: 'file' | 'directory'): ProtectionCheck => {
    if (isWindows) {
      if (!windowsPrincipal) {
   ...
   */
  const inspectExistingFile = (path: string): { kind: 'missing' } | ProtectionCheck => {
    if (isWindows) {
      try {
        lstatSync(path);
      } catch (error) {
        if ((error as { code?: string } | null)?.code === 'ENOENT') return { kind: 'missing' };
        return { ok: false, detail: fileSystemDetail(error) };
      }
      if (!windowsPrincipal) {
        return { ok: false, detail: 'Windows did not report which account is running this command' };
      ...
      const inspected = runIcacls(buildAclInspectCommand(path));
      if (!inspected.ok) return { ok: false, detail: 'its access control list could not be read with icacls' };
      return verifyAclOutput(inspected.stdout, windowsPrincipal, path);
    }
  ...
  let usable: ProtectionCheck | null = null;

  const ensureDirectory = (): ProtectionCheck => {
   ...
   */
  const inspectDirectory = (): ProtectionCheck => {
    ...
    try {
      ...
      if ((error as { code?: string } | null)?.code === 'ENOENT') return { ok: true };
      return { ok: false, detail: fileSystemDetail(error) };
    }
    ...
    return checkPosixProtection(
      ...
      { kind: 'directory', uid: process.getuid?.() ?? facts.uid },
      { ignoreSharedAccess: true },
    );
  };
  ...
  return {
    ...
    read(account) {
      ...
      if ('kind' in inspected) return { kind: 'missing' };
      if (!inspected.ok) {
      ...
      try {
        const parsed = JSON.parse(body) as { version?: unknown; secret?: unknown };
