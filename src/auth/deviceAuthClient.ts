...
import {
  ...
  type PersonalTokenIdentity,
} from './personalTokenClient.js';
 ...
 */
export async function startDeviceAuthorization(input: DeviceAuthStartInput): Promise<DeviceAuthorizationStart> {
  ...
  try {
    ...
    throw new PersonalTokenError(
      'TRANSIENT',
      `Could not reach ${baseUrl} to start device authorization: ${error instanceof Error ? error.message : String(error)}.`,
    );
  }
  ...
  if (response.status === 404) {
    throw new PersonalTokenError(
      'TRANSIENT',
      `${baseUrl} does not support device authorization (it is running an older AgentTeams API). ` +
  ...
  if (response.status === 503) {
    throw new PersonalTokenError(
      'TRANSIENT',
      `${baseUrl} has device authorization turned off because the server has no APP_URL configured. ` +
  ...
  if (
    ...
    throw new PersonalTokenError(
      'MALFORMED_RESPONSE',
      'The server returned an unexpected device authorization payload.',
    );
  }
  ...
  return {
    ...
    verificationUriComplete:
      typeof data.verificationUriComplete === 'string' ? data.verificationUriComplete : data.verificationUri,
 ...
 */
export async function pollDeviceAuthorization(input: DeviceAuthPollInput): Promise<DeviceAuthResult> {
  ...
  while (true) {
    ...
    if (now() >= deadline) {
      throw new PersonalTokenError(
        'INVALID_GRANT',
        'The device code expired before it was approved. Run the command again to get a new code.',
      );
    }

    const outcome: DevicePollOutcome = await client.pollDeviceToken(input.start.deviceCode);
    ...
    switch (outcome.kind) {
      ...
      case 'invalid':
        throw new PersonalTokenError(
          'INVALID_GRANT',
          'The device code is no longer valid. Run the command again to get a new code.',
        );
      case 'transient':
        ...
        if (transientFailures >= MAX_TRANSIENT_FAILURES) {
          throw new PersonalTokenError(
            'TRANSIENT',
            `Lost contact with ${input.apiUrl} while waiting for approval: ${outcome.detail}.`,
          );
        }
