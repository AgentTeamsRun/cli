...
export class RefreshLockUnavailableError extends Error {
  constructor(
    ...
    super(
      `Could not maintain the credential rotation lock at ${lockPath}: ${
 ...
 */
export const noopRefreshLock: RefreshLock = {
  withLock: (run) => run(),
};
...
export function createFileRefreshLock(slot: string, options: CreateFileRefreshLockOptions = {}): RefreshLock {
   ...
   */
  const readHeldSince = (): number | null => {
    try {
      const record = JSON.parse(readFileSync(lockPath, 'utf-8')) as LockRecord;
   ...
   */
  const publishLockFile = (owner: string, record: LockRecord): void => {
  ...
  const acquire = async (): Promise<string | null> => {
    ...
    for (;;) {
      ...
      const record: LockRecord = { owner, startedAt: now() };
  ...
  const release = (owner: string | null): void => {
