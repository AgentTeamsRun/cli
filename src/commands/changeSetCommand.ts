...
export async function executeChangeSetCommand(
  ...
  switch (action) {
    ...
    case 'get': {
      ...
      if (!id) throw new Error('--id is required for change-set get');
      return getChangeSet(context, id);
    }
    case 'create': {
      const title = toNonEmptyString(options.title);
      if (!title) throw new Error('--title is required for change-set create');
      return createChangeSet(context, title, toNonEmptyString(options.description));
    }
    case 'update': {
      const id = toNonEmptyString(options.id);
      if (!id) throw new Error('--id is required for change-set update');
    ...
    case 'delete': {
      const id = toNonEmptyString(options.id);
      if (!id) throw new Error('--id is required for change-set delete');
      return deleteChangeSet(context, id);
    }
    case 'add-item': {
      const changeSetId = toNonEmptyString(options.changeSetId);
      if (!changeSetId) throw new Error('--change-set-id is required for change-set add-item');
      const mergeOrder = toPositiveInteger(options.mergeOrder);
      if (!mergeOrder) throw new Error('--merge-order must be a positive integer for change-set add-item');
      return addChangeSetItem(context, changeSetId, mergeOrder, options);
    }
    case 'remove-item': {
      const changeSetId = toNonEmptyString(options.changeSetId);
      if (!changeSetId) throw new Error('--change-set-id is required for change-set remove-item');
      const itemId = toNonEmptyString(options.itemId);
      if (!itemId) throw new Error('--item-id is required for change-set remove-item');
      return removeChangeSetItem(context, changeSetId, itemId);
    }
    default:
      throw new Error(
        `Unknown change-set action: ${action}. Use create, list, get, update, delete, add-item, or remove-item.`,
      );
  }
}
