 ...
 */
const canonicalizeWorktreePath = (worktreePath: string): string => {
...
export const computeWorktreeLocalKey = (worktreePath: string): string =>
  createHash('sha256').update(canonicalizeWorktreePath(worktreePath)).digest('hex');

/** worktree lifecycle을 알려 온 호스트. eventId prefix로 남아 이벤트 출처를 구분한다. */
export type WorktreeEventHost = 'orca' | 'herdr';

export const createDefaultWorktreeEventId = (
  event: WorktreeLifecycleEvent['event'],
  host: WorktreeEventHost = 'orca',
): string => `${host}:${event.toLowerCase()}:${randomUUID()}`;
 ...
 */
const HERDR_EVENT_ALIASES: Record<WorktreeLifecycleEvent['event'], string[]> = {
 ...
 */
export const parseHerdrWorktreeEvent = (
  env: NodeJS.ProcessEnv,
  expected: WorktreeLifecycleEvent['event'],
): HerdrWorktreeEvent => {
  ...
  if (!raw) {
    throw new Error(
      `HERDR_PLUGIN_EVENT_JSON is missing. Run this command from a herdr '${expectedName}' plugin event hook.`,
    );
  }

  let parsed: {
    event?: unknown;
    data?: { worktree?: { path?: unknown; branch?: unknown }; workspace?: { worktree?: Record<string, unknown> } };
  };
  try {
  ...
  const receivedName =
    (typeof env.HERDR_PLUGIN_EVENT === 'string' && env.HERDR_PLUGIN_EVENT.trim()) ||
...
const defaultGitReader: WorktreeGitReader = {
...
export const resolveCwdIdentity = (cwd: string, git: WorktreeGitReader = defaultGitReader): WorktreeIdentity => {
...
export const resolveHerdrIdentity = (
  herdrEvent: HerdrWorktreeEvent,
  event: WorktreeLifecycleEvent['event'],
  git: WorktreeGitReader = defaultGitReader,
): WorktreeIdentity => ({
  host: 'herdr',
 ...
 */
export const buildWorktreeLifecyclePayload = (
  event: WorktreeLifecycleEvent['event'],
  identity: WorktreeIdentity,
  options: Record<string, unknown>,
): WorktreeLifecycleEvent => {
  const repositoryId =
    typeof options.repositoryId === 'string' && options.repositoryId.trim() ? options.repositoryId.trim() : undefined;
  ...
  const localKey =
    (typeof options.localKey === 'string' && options.localKey.trim()) || computeWorktreeLocalKey(identity.worktreePath);
  const eventId =
