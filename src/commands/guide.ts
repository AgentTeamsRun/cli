 ...
 */
export async function guideGet(options: { recordKind?: unknown; cwd?: string }): Promise<unknown> {
  ...
  let guide = loadLocalPlatformGuide(recordKind as GuideRecordKind, projectRoot);
  if (!guide) {
    const { apiUrl, headers } = await getApiConfigOrThrow({ cwd: options.cwd });
    guide = await resolvePlatformGuide(recordKind as GuideRecordKind, { projectRoot, apiUrl, headers });
  }
...
export async function executeGuideCommand(action: string, options: any): Promise<unknown> {
  switch (action) {
    case 'list':
      return {
