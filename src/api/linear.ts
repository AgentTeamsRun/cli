...
const withProjectId = (headers: any, projectId?: string) =>
  projectId ? { headers, params: { projectId } } : { headers };
...
const issueUrl = (apiUrl: string, issueId: string, suffix = '') =>
  `${apiUrl}/api/linear/issues/${encodeURIComponent(issueId)}${suffix}`;
