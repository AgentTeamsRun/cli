...
describe('mcp client registry', () => {
  it('exposes exactly the twelve supported clients and excludes Orca and GEMINI', () => {
    expect(MCP_CLIENTS.map((client) => client.id)).toEqual([...MCP_CLIENT_IDS]);
    expect(MCP_CLIENTS).toHaveLength(12);
...
describe('mcp config output', () => {
  ...
  it.each(['user', 'project'] as McpScope[])(
    ...
    (scope) => {
      ...
      expect(output.json.clients).toHaveLength(12);
