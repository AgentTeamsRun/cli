...
describe('mcp exact list and missing detail tools', () => {
  ...
  it('exposes exactly ten list, eleven get, and one search tool without duplicates', async () => {
    ...
    // 23 shared read tools + 2 CLI-local tools + 17 write tools; agentteams_guide_get
  ...
  it('classifies the complete catalog into stable public profiles and reports its definition budget', () => {
    ...
    expect(new Set(catalog.map(({ name }) => name)).size).toBe(42);
    ...
    expect(budget).toMatchObject({ toolCount: 42 });
